...
- `GET /api/audit` → the audit report. Public read, same stance as `/api/banned`: everything in it is derived from public events (the page defers fetching it until admin login, but that is UX, not security). Response: `{"generated_at": <unix>, "relay_url": "<verbatim from config or "">", "warning": "<string or null>", "notices": [...], "activity": [...]}`.
  ...
  **Author chunking**: every authors-based scan (2 and 4) is invoked in batches of ≤1000 authors per `strfry scan` subprocess, run sequentially, with results concatenated and deduped by event `id` before handing them to `lib86/audit.py`. Rationale: a single argv element listing thousands of hex pubkeys can exceed Linux's `MAX_ARG_STRLEN` (128 KiB per argv element), which fails with `OSError: [Errno 7] Argument list too long` — this hit production at ~1900 authors in scan 2. Total authors per audit is hard-capped at 10000 (10 batches); if the cap truncates the input, the audit still succeeds and `warning` notes `"audit sampled first 10000 of N authors (<scan name>)"` instead of failing. Each batch gets the per-scan timeout individually — a slow batch doesn't steal timeout budget from the others.
...
**Mandatory SCALE STAGE**: before every release, run `/api/audit` against a strfry instance seeded with ≥3000 distinct-author kind-10002 events (a prod-like container — real strfry binary, real LMDB, real subprocess argv — not the pure-function fixtures above) and assert the response comes back with `warning: null`, at least one notice, within 90 seconds. Rationale: toy-scale smoke tests miss OS-limit and timeout bugs entirely — production hit binary-not-on-PATH, wrong-cwd LMDB-open, and `OSError: [Errno 7] Argument list too long` failures that only surface in a prod-like container at prod-like scale, never in unit tests against small fixtures.
