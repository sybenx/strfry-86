...
4. Manual ban form (admin only, hidden until login), followed by the record lines (see below)
...
8. "Select all" checkbox (admin only, hidden until login) then the "Unban selected" button (admin only, hidden until login), followed by a `<b>` live count of checked entries — "<n> selected", empty at zero (so it renders nothing while logged out) — updated on every checkbox change, select-all toggle, and list re-render. These sit directly above the list they operate on; the admin scrolls down, ticks boxes, scrolls back up to these controls rather than to the top of the page
9. The ban list
...
- "Unban selected" button → build kind-27235 event with `u` + `method` tags, `window.nostr.signEvent`, POST `/api/unban`, add one unban record line (see below), re-fetch, re-render.
...
- **Record lines** (admin only, rendered directly below the ban form): a persistent log of admin actions, each line being a plain-text label, an "Undo" button, and a "↩" dismiss button as the rightmost element of the line. Three kinds:
  - **Ban records**: after a successful manual ban, one line per banned pubkey (added after the re-fetch, so the entry's name/npub are available): "banned <name> <npub>". Undo signs and POSTs a single-pubkey `/api/unban`.
  ...
  - **Report records**: derived, not stored — for every entry in `/api/banned` whose `report_event_id` is non-null (i.e. banned by the admin's kind-1984 report) and whose id (`<report_event_id>:<pubkey>`) is not in the dismissed list: "reported <name> <npub> — <report_type>". Undo signs and POSTs a single-pubkey `/api/unban` (the line then disappears with the ban); ↩ adds the id to a localStorage dismissed list (capped at the 1000 newest).

  Ban and unban records persist in localStorage (capped at the 200 newest) until their ↩ is clicked or their Undo succeeds — they survive reloads and sessions. A successful Undo removes/dismisses the line, re-fetches, re-renders; a failed Undo re-enables the button and reports via the status line. Record lines re-render on every list re-render and only when logged in as admin (localStorage is per-browser UI state, not server state — nothing about records is served). Name and npub are inserted via `textContent` like everywhere else.
