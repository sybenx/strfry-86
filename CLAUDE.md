...
/config/strfry86/
  ...
  server86.log       # RUNTIME-CREATED by server86 (error log; see /api/audit failure reporting). Never in manifest, never touched by the updater.
```
...
- `GET /` → `admin.html`.
- `GET /api/banned` → `{"admin": "<hex>", "contact_appeal": "<string>", "banned": [{"pubkey", "npub", "banned_at", "reason", "report_type", "name"}]}`. Public read is fine. `contact_appeal` is echoed verbatim from `config.json`, or `""` if the key is absent, null, or not a string — this endpoint must never 500 over a hand-edited config. Re-read it from `config.json` on mtime change (the same cheap once-per-second mtime check `lib86/blacklist.py` uses) so an operator's hand-edit takes effect on the next page load without restarting anything. `name` is resolved server-side from the LOCAL strfry database: run `strfry --config /config/strfry.conf scan '{"kinds":[0],"authors":[<uncached hex>]}'` via subprocess (the conf path is a constant at the top of the file; the strfry BINARY path is DISCOVERED once at startup, not assumed on PATH: use `shutil.which("strfry")`, else the first path in `("/app/strfry", "/usr/local/bin/strfry", "/usr/bin/strfry", "/strfry")` that exists and is executable — dockurr/strfry ships the binary at `/app/strfry`, which is NOT on PATH for a detached process, while the official image installs to `/usr/local/bin`. Cache the discovered path for the process lifetime and use it for every scan (name lookup and audit alike). If no candidate is found, don't crash: name lookup returns `name: null` as usual, and the audit `warning` and server86.log state `strfry binary not found` listing the candidates tried, so the operator can report the actual location for addition to the list. scan is a read-only LMDB read, safe while the relay runs), parse each event's `content` JSON, take `display_name || name`. Cache results in an in-memory dict `{pubkey: (name, checked_at)}`; re-query only pubkeys that are uncached or were misses older than 24h, and batch all of them into ONE scan call per request. If the subprocess fails for any reason (binary missing, bad path, timeout of a few seconds), log to stderr and return `name: null` — the endpoint must never break because name lookup broke.
...
- `GET /api/audit` → the audit report. Public read, same stance as `/api/banned`: everything in it is derived from public events (the page defers fetching it until admin login, but that is UX, not security). Response: `{"generated_at": <unix>, "relay_url": "<verbatim from config or "">", "warning": "<string or null>", "notices": [...], "activity": [...]}`.
  ...
  **Caching / execution model**: the report is computed by a helper that runs the `strfry scan` subprocesses (same binary/conf constants and safety notes as the name lookup) and hands parsed events to `lib86/audit.py`. Every scan — here and in name lookup — is invoked as an argv LIST with `shell=False`: `[<discovered binary>, "--config", "/config/strfry.conf", "scan", json.dumps(<filter>, separators=(",", ":"))]`. The filter is exactly ONE argv element containing raw compact JSON. Never build the command as a single string, never `.split()` a command string (json.dumps output contains no argv-safe boundaries), never wrap the filter in quotes, never backslash-escape its quotes (escaped quotes reach strfry verbatim and it exits 1), never `shell=True`. `strfry.conf`'s `db` is conventionally a path relative to wherever the relay process itself was launched (e.g. dockurr/strfry runs `./strfry` from `/app` with `db = "./strfry-db/"`), which is NOT necessarily server86's own cwd — plugin86 spawns server86 with `cwd=SCRIPT_DIR` (`/config/strfry86`), so a scan subprocess with no cwd override resolves the relative db path against the wrong directory and strfry exits 1 with `mdb_env_open: No such file or directory`. Every scan subprocess (name lookup and audit alike) MUST therefore pass `cwd=`, resolved by locating the running strfry relay process via `/proc` (match on the discovered binary's basename plus a `relay` argv element) and reading `/proc/<pid>/cwd`; fall back to the discovered binary's parent directory if no relay process is found. Cache the discovered cwd for the process lifetime, re-deriving only if the located pid disappears. Capture stderr and include its tail (last ~300 chars) in the failure report when the process exits nonzero — strfry's loguru output puts a startup banner first and the actual error (tao::json parse or LMDB env open) as the LAST line, so truncating from the head discards the error itself; always slice from the end. Serve from an in-memory cache; if the cache is older than 10 minutes, serve the STALE copy immediately and kick off a recompute in a background thread (single-flight — never two concurrent recomputes). First-ever request before any cache exists may compute synchronously with a hard timeout of a few seconds; on timeout or any subprocess failure, return an empty skeleton (`notices: []`, `activity: []`) with `warning` set. The few-second hard timeout applies ONLY to that first synchronous compute; background recomputes use a 60-second per-scan timeout — they run in a thread, so "never hang" is already satisfied for the request path. This endpoint must never 500 and never hang.

  **Failure reporting**: `warning` must never be a bare generic string. On any scan failure it names which scan failed and includes the exception's type and message — built from the stderr TAIL per the cwd/tail rules above, not the head — truncated to an overall cap of ~600 chars (e.g. `audit scan 3 (activity) failed: TimeoutExpired: ...`). The same line, prefixed with a unix timestamp, is also appended to `/config/strfry86/server86.log` — append-only, size-capped: when the file exceeds 64 KB, atomically rewrite it to its last 32 KB (write to `.tmp`, `os.replace`). The operator's environment may offer no usable shell (server86's stderr goes to devnull by design), so the `warning` field and this log file are the ONLY observable error channels; every caught exception in server86 (audit scans, name lookup, request handlers) is written to the log, and logging failures are themselves swallowed — the log must never break the endpoint it exists to explain.

  **Scans** (the only I/O, all read-only LMDB):
  1. `{"kinds":[10002,10050]}` — all stored relay lists. Small on any relay.
  2. Per-author-bounded footprint scan over the distinct authors from scan 1 (minus admin, minus already-banned) — see **Scan 2 (footprint) is output-bounded, not just author-chunked** below. This replaced a single unbounded `{"authors":[...]}` filter that streamed each author's ENTIRE stored history; that design hit production at scale (real accounts with thousands of stored events turned one batch into gigabytes of stdout and a 60s-per-batch timeout — see below).
  3. `{"since": <now - 28 days>}` — for activity sparklines.
  4. Per-author-bounded existence scan over currently banned pubkeys (limit 1 per author, same array-of-filters mechanism as scan 2) — purge_pending only needs "does this banned pubkey still have ≥1 stored event," not their full history either.

  Any scan whose `authors` list would be empty (scans 2 and 4 on a quiet or ban-free relay) is SKIPPED outright and treated as returning zero events — never invoke `strfry scan` with `"authors": []`. An empty-authors filter is at best undefined and at worst an instant subprocess failure that blanks the whole report.

  **Scan 2 (footprint) is output-bounded, not just author-chunked.** Author chunking (below) bounds argv size; it does nothing about output size — a single real account with thousands of stored events (kind 1 notes, etc.) still streams its entire history through one filter, and that is what actually timed out in production, not the argv length. The fix has two parts:

  - **What the detector actually needs**: `build_ghosts_notice` in `lib86/audit.py` only checks whether an author's footprint kinds are a subset of `GHOST_ALLOWED_KINDS = {0, 3, 10002, 10050}` — it never looks at event content or count. It needs a small bounded sample of an author's stored kinds, not their full history.
  - **The bound, exploiting strfry's own replaceable-event semantics**: kinds 0, 3, 10002, and 10050 are ALL strfry-enforced replaceable (verified against the real binary: importing 3 kind-0 events for one pubkey leaves exactly 1 stored) — so an author's entire ghost-kind footprint is capped at `len(GHOST_ALLOWED_KINDS)` = 4 stored events, ever, no matter how many times they republished. Scan 2 therefore issues one `{"authors":[pk],"limit":5}` filter PER AUTHOR (`limit` is per-FILTER in strfry, not per-author, so a single combined `{"authors":[...many...],"limit":5}` filter would only cap the whole batch's output, not each author's — hence one filter object per author, submitted as a JSON ARRAY of filters in a single `strfry scan` invocation; each filter's limit is applied independently). This is **exact, not a sampled approximation**: if fewer than 5 events come back for an author, that IS their entire stored history; if all 5 come back, at least one must be a non-ghost kind by pigeonhole (only 4 replaceable slots exist), so ghost/non-ghost classification never changes — only the amount of data streamed to get there does. No semantic change versus the original "ever published anything else" definition.
  - Authors whose batch still fails (timeout, non-zero exit, or the stdout byte cap below) are NOT treated as ghosts by default (an absent footprint would otherwise look identical to "published nothing else"): they're collected into an `unknown_pubkeys` set that `build_ghosts_notice` excludes from ghost candidacy entirely, and the failure is folded into `warning` once (not once per batch). Authors dropped by the `AUTHOR_HARD_CAP` sampling truncation (below) get the same treatment for the same reason — a huge relay must degrade to "we don't know about the excess", never to "the excess are all ghosts."
  - **General safety net**: independent of the per-author limit, every `strfry scan` subprocess (this and every other scan, including name lookup) is run through a shared helper that kills the process and raises if stdout exceeds a fixed byte cap (16 MiB), so a future filter change that drops a `limit` or an `authors` bound can't reintroduce unbounded streaming silently — it fails loudly and degrades that one batch instead.

  **Author chunking**: every authors-based scan (2 and 4) is invoked in batches of ≤1000 authors per `strfry scan` subprocess, run sequentially, with results concatenated and deduped by event `id` before handing them to `lib86/audit.py`. Rationale: a single argv element listing thousands of hex pubkeys can exceed Linux's `MAX_ARG_STRLEN` (128 KiB per argv element), which fails with `OSError: [Errno 7] Argument list too long` — this hit production at ~1900 authors in scan 2 in the old authors-list format. The new one-filter-per-author array format is more verbose per author (~90 bytes vs ~67) but 1000 authors is still only ~90 KiB per invocation, safely under the 128 KiB limit. Total authors per audit is hard-capped at 10000 (10 batches); if the cap truncates the input, the audit still succeeds and `warning` notes `"audit sampled first 10000 of N authors (<scan name>)"` instead of failing — and (for scan 2) the truncated authors are folded into `unknown_pubkeys` per above. Each batch gets the per-scan timeout individually — a slow batch doesn't steal timeout budget from the others, and a failed batch degrades only that batch's authors rather than aborting the scan.
  ...
  **Notices** (assembled in `lib86/audit.py` from the scan results; each is `{"type", "text", "pubkeys": [<hex>...], "suggested_reason"}` — `text` is a complete plain-language sentence built server-side so the admin page and any external consumer read the identical finding; the pubkey array is the actionable payload):
  1. `type: "ghosts"` — pubkeys from scan 1 whose entire footprint in scan 2 is kinds ⊆ {0, 3, 10002, 10050}, excluding admin, already-banned, and `unknown_pubkeys` (authors whose footprint scan 2 batch failed or was sampling-truncated — see above; treated as "we don't know," never as ghosts). These published a relay list (and at most a profile/follow list) and nothing else — manufactured-key signature. Text pattern: "N pubkeys have published only a relay list (M point at this relay)." M is the count whose 10002/10050 contains an `r` tag matching `relay_url` after normalization on both sides; omit the parenthetical when `relay_url` is unset. `suggested_reason: "audit: relay-list ghost"`.
...
`tools/make_bundle.py` regenerates BOTH `manifest.json` (sha256 over every deployable file, sorted keys, trailing newline) and `strfry86-bundle.tar.gz` (every deployable file plus the manifest, deterministic member order). Any commit that changes a deployable file MUST regenerate both — a stale manifest or stale bundle makes the updater skip, reject, or install outdated files. The bundle is committed to the repo so it has one stable raw URL. test.sh should verify that the manifest matches the working tree AND that the committed bundle's contents hash-match the manifest, in addition to piping crafted JSONL through `python3 plugin86.py` and asserting accept/reject for: normal event accepted; banned author rejected; admin 1984 bans its p-tags; non-admin 1984 does not ban; admin pubkey cannot be banned. test.sh must also exercise `lib86/audit.py` directly on crafted event fixtures (no strfry binary needed — the functions are pure): a pubkey with only a 10002 classifies as ghost; a pubkey with a 10002 plus a kind 1 does not; admin and already-banned pubkeys are excluded from ghosts; a pubkey in `unknown_pubkeys` with an empty (or absent) footprint is excluded from ghosts rather than defaulting to ghost status; `normalize_relay_url` collides `wss://X.Y/`, `wss://x.y`, and `wss://x.y//`; ≥10 same-hour relay lists from distinct authors produce a burst notice; 5 identical sorted r-tag sets produce a fingerprint notice; malformed events (missing tags, tags not a list) are skipped without raising.

**Mandatory SCALE STAGE**: before every release, run `/api/audit` against a strfry instance seeded across BOTH volume dimensions (a prod-like container — real strfry binary, real LMDB, real subprocess argv — not the pure-function fixtures above), not just author count:

- ≥10000 distinct-author kind-10002 events (PoW-style pubkeys, 1-3 relay-list events each) — the swarm/ghost side.
- ≥20 additional "real" authors with ≥5000 stored content events each (≥150000 total events across the seed) — the high-volume-real-account side. This is the dimension that mattered most: the swarm side alone never reproduced the production failure, because ghost accounts have almost nothing to stream. It was scan 2 streaming a handful of REAL accounts' entire histories, unbounded, that produced gigabytes of stdout and a 60s-per-batch timeout.

Assert the response comes back within 90 seconds, with at least one notice, real authors never flagged as ghosts, and `warning` either `null` or (expected once total candidate authors exceed `AUTHOR_HARD_CAP`) the documented `"audit sampled first N of M authors"` note — never a scan-failure or timeout message. Rationale: toy-scale smoke tests miss OS-limit and timeout bugs entirely — production hit binary-not-on-PATH, wrong-cwd LMDB-open, `OSError: [Errno 7] Argument list too long`, and (most recently) per-author output volume in scan 2 — failures that only surface in a prod-like container at prod-like scale on BOTH dimensions, never in unit tests against small fixtures or author-count-only smoke tests.
