...
- **The server never opens an outbound network connection. Ever.** All third-party contact happens from the admin's logged-in browser. There are exactly two such contacts, and both are listed here or they do not exist:
  1. **kind-0 profile lookup against a fallback chain of public relays** (`wss://purplepag.es`, `wss://indexer.coracle.social`, `wss://user.kindpag.es`, `wss://relay.nos.social`, `wss://relay.ditto.pub`, `wss://relay.noswhere.com`, tried in that order, each one only on a CONNECT failure of the one before it), for pubkeys that could not be resolved locally. Results come back through ONE authenticated endpoint (`POST /api/names`) which cryptographically verifies every displayable claim before storing it.
...
- **Record lines** (admin only, every page, rendered from shared code over the same localStorage): a persistent log of admin actions.
  ...
  **Element order within each line is fixed**: the "↩" dismiss button LEFTMOST, then the label, then the "Undo" button, then the **full npub as plain text, last**. The npub wraps onto a second line naturally — that wrap is the design, not a defect to style away. This ordering keeps ↩ and Undo apart with the label between them and leaves Undo flanked by inert text on BOTH sides; previously Undo was the line's last element and sat adjacent to the next record's ↩ across the line break. The npub is NOT a link — the profile entry field is how you go look at one, so a single-pubkey record line has exactly two touch targets no matter how long it grows. The locked CSS block permits no margin, padding, or gap rules, so DOM order is the only lever; flex and gap were considered here and rejected. Never render a record line with an empty label, since the label supplies the separation.

  **A record line covering 2+ pubkeys is the one exception to "exactly two touch targets".** It has no trailing npub — ambiguous which one — and in that slot gets a third control instead: a "▸"/"▾" expand arrow, rightmost, that toggles a nested list below the line, one row per pubkey, each row built the same way as everywhere else — the display name in `<b>` when known, the nip05 as plain text after it, then the npub. Collapsed is the default and the only state that counts toward anything; expanding is pure client-side disclosure of data already on the line's own record, with no server round-trip and nothing stored about the expanded/collapsed state itself.
  ...
  **Names are resolved at RENDER time, not capture time.** localStorage stores only `pubkey`, action, timestamp, and (for unbans) the original reason; `name` and `nip05` are looked up by pubkey against the current `/api/banned` payload every time the lines are drawn. Lines already sitting undismissed in an admin's browser therefore gain names the moment a build ships — no stored-key backfill, no migration. Unban records are the one exception and must snapshot `name`/`nip05` at capture, because the server no longer knows those pubkeys after the unban; older unban records simply lack the fields and render name-only.
  ...
  - **Ban records**: after a successful ban from ANY page, ONE line per ban ACTION, not per pubkey. A single-pubkey ban renders "banned **<name>** <nip05>", then Undo, then the npub, exactly as above. A bulk ban — the manual multi-npub form on `bans.html`, "Ban selected" on `authors.html`, or "Ban selected" on `domain.html` — renders one grouped line instead of one line per pubkey: "banned <n> pubkeys", followed by the shared reason when one was given ("banned 5 pubkeys — spam links"), then Undo, then the expand arrow. The reason is shared rather than per-pubkey because a bulk ban has exactly one reason input for the whole batch. Undo signs and POSTs ONE `/api/unban` carrying every pubkey in the record.
  - **Unban records**: after a successful "Unban selected", ONE line per action: "unbanned **<name>** <nip05>" plus the trailing npub for a single pubkey, or "unbanned <n> pubkeys" with the expand arrow in place of a npub for more. The unbanned entries (pubkey, npub, name, nip05, reason) are captured from the list before the re-fetch discards them; Undo re-bans all of them via `/api/ban` with each original reason (re-bans therefore get `report_type = "manual"`).
  ...
  **At most the 20 newest record lines of all kinds combined are ever rendered**, newest first; everything older than that is auto-dismissed — dropped from the rendered set and, for ban/unban/reason records, deleted from localStorage. That 20-line cap is hard, never a "show more" affordance on the STORED set: a relay with 400 reported bans still only ever holds 20 in play. Stored records keep the 200 newest; the 1000-id dismissed list stays as a backstop it should never approach.

  **Of those (up to) 20, only the newest `S86_RECORDS_INLINE` (3) render inline, always visible with no click required.** The rest sit inside a `<details>` reading `recent activity (<n> older) ▸`, arrow flipping to `▾` on open via the element's own `toggle` event — the same iconography as the multi-pubkey expand arrow elsewhere on the line. This is presentation only, over the already-capped 20: it changes nothing about what's stored, what's dismissed, or the 20-line ceiling itself, and exists because these lines sit ABOVE the list in the space reserved for controls that must stay reachable — three lines announce that history exists without pushing the ban form, the filter, and the list itself off the screen on a busy relay. Records survive reloads, sessions, and navigation between pages, and render only when logged in as admin.
...
- **Name-resolution flow** (admin only; one of exactly two paths by which this project contacts a machine the operator does not run — and it runs in the admin's browser, never in server86):
  ...
  - If non-empty: open a WebSocket to the first relay in the fallback chain (`wss://purplepag.es`, `wss://indexer.coracle.social`, `wss://user.kindpag.es`, `wss://relay.nos.social`, `wss://relay.ditto.pub`, `wss://relay.noswhere.com`), send a single batched REQ `{"kinds":[0],"authors":[<unresolved hex>]}`, collect until EOSE or a 5s timeout; on failure to CONNECT, try the next relay in the chain, in order, until one connects or the chain is exhausted. Then POST everything received, verbatim and unparsed, plus the list of pubkeys actually queried, to `/api/names`. Re-fetch `/api/banned`, re-render, re-apply the filter.
  ...
  - **Status line**, plain text, rendered whenever the flow ran: `resolved <n> locally, <m> via external lookup, <k> unresolved`. The outbound connection must be visible in the page itself, not only in a network tab.
