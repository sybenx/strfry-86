...
The public landing page is **Activity** (`/`) — a live feed of recent events showing only kind, timestamp, and event id (no pubkeys or content). Nav is consistent across pages: Activity, Stats, Report, Authors, Userlist, Audit, Bans. Search (Modifier-K) accepts npub, hex, or domain.
...
`/api/subscribers` needs to know your relay's own address, which strfry doesn't otherwise expose to itself. Set it on the `/report` page, in the field right above the subscriber scan — it's stored in `config.json`, not `strfry.conf`, and strfry itself never reads it. Without it, `/api/subscribers` returns an empty result and says why, rather than guessing.
